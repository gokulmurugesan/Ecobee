...
app.use('/generate', function(req, res) {
    ...
    runProgram(array).done(function (err) {
...
// Production error handler will have no stacktraces leaked to user
app.use(function(err, req, res, next) {
  res.status(err.status || 500);
  res.render('error', {
    message: err.message,
...
var noDataThermostats = "";                 //Saves all thermostats with no data as a string
...
function getDemandResponseThermostats(token) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        request.get(baselink + 'hierarchy/set?format=json&body={"operation":"list","setPath":"/' + setName + '","recursive":true,"includeChildren"=true,"includePrivileges":false,"includeThermostats":true}', {
            ...
            //Upon receiving a response
            if (data.status.code == '3') { error = "The Set Name inserted is not available for this account. Please input a valid Set Name"; reject(error); } 
            else { resolve(data); }
        });
    });
}
...
function participationData() {
    return new Promise(function (resolve, reject) {
        ...
        var thermostatIDs = "315697894149,314705625832,315697894150,315697894151,315697894152,315697894153,315697894154,315697894155,315697894156,315697894157,315697894158,315697894159,315697894160,315697894161,315697894162,315697894163".split(','); //DEBUGGING REMOVE LATER;
                    ...
                    $.csv.toObjects(csv, {}, function (err, data) {
                        ...
                        if (err) { reject("Cannot parse " + csvFile + " correctly. Please ensure that file is formatted correctly and closed."); }
                            ...
                            async.forEach(data, function (row, callback) {
                                ...
                                if ((row.zoneAveTemp == "") && (row.zoneCoolTemp == "") && (row.zoneHeatTemp == "")) {
                                    emptyRowCount++;
                                }
                                callback(); //Next Row
                            }, function (err) {
                                ...
                                if (emptyRowCount > 3) { noDataThermostats += id + ","; }
                                else if (participationRows === data.length) { participatingThermostats += id + ","; } 
...
function participationListCSV() {
    return new Promise(function (resolve, reject) {
        ...
        var noData = noDataThermostats.split(",");
                    ...
                    fs.readFile(currentThermostat, 'UTF-8', function (err, thermostatCSV) {
                        if (err) {
                            ...
                            $.csv.toObjects(thermostatCSV, {}, function (err, data) {
                                ...
                                var rowNumber = 0;
                                async.forEach(data, function (row, callback) {
                                    //When populating array first time around
...
function runProgram (details) {
    return new Promise(function (resolve, reject) {
        ...
        //Reinitialize Variables Everytime program runs
        thermostatList = "", thermostatListRequest = [], participatingThermostats = "", nonParticipitatingThermostats = "", optOutThermostats = "", noDataThermostats = "", kWFactorData = {};
        ...
        .then(function (success) {
        saveFile("NoData.csv", dirDemandResponse, noDataThermostats.split(",").join("\n"))
        .then(function (success) {
...
//            .catch(function (err) { error = err;  reject(error); })})
//            .catch(function (err) { error = err;  reject(error); })})
//            .catch(function (err) { error = err;  reject(error); })})
//            .catch(function (err) { error = err;  reject(error); })})
//            .catch(function (err) { error = err;  reject(error); })})
